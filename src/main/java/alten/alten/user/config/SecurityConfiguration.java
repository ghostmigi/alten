...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.cors(cors -> cors.configurationSource(corsConfigurationSource())) // Enable CORS here
                ...
                .authorizeHttpRequests(request -> request
                        ...
                        .requestMatchers("/api/v1/cart").hasAnyAuthority(Role.ADMIN.name(), Role.USER.name())
                        .requestMatchers("/contact").hasAnyAuthority(Role.ADMIN.name(), Role.USER.name())
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOrigins(Arrays.asList("http://localhost:3000")); // Your React app URL
        config.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE", "OPTIONS"));
        config.setAllowedHeaders(Arrays.asList("Authorization", "Content-Type"));
        config.setAllowCredentials(true); // Allow credentials (for cookies or JWT)
